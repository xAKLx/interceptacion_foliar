...
#' @export
filtrarPorIgualdad <- function(fila, columna, valor) {
...
#' @export
filtrarPorRango <- function(fila, columna, valorMinimo, valorMaximo
                            , destino) {
  
}

#' Crea un backup de la data
#' 
#' @param data Objeto a guardar creado por la función de importar o 
#' importarBackup.
#' @return nombre del backup
crearBackup <- function(data) {
  
}

#' Restaura un backup dado un nombre
#' @param name string representativo del nombre del backup.
#' @return objeto con la informacion del backup
importarBackup <- function(name) {
  
}

#' Obtiene un vector con todos los nombres de los backups disponibles
#'
#' @return vector con los nombres de todos los backups. 
obtenerBackups <- function() {
